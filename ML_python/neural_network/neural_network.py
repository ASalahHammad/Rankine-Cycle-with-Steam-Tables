...
class Network(object):
    ...
    def __init__(self, sizes):
        ...
        self.size = sizes
        # if pth.isfile('w.npy'):
        #     if pth.getsize('w.npy')>0:
        #         w_file = open('w.npy','rb')
        #         self.weights = pickle.load(w_file,encoding='latin1')
        # else:
        self.biases = [np.random.randn(y,1) for y in sizes[1:]]
        # if pth.isfile('b.npy'):
        #     if pth.getsize('b.npy')>0:
        #         b_file = open('b.npy','rb')
        #         self.biases = pickle.load(b_file,encoding='latin1')
        # else:
        self.weights = [np.random.randn(y,x) for x, y in list(zip(sizes[:-1], sizes[1:]))] # python 3
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None):
        ...
        n = len(training_data)
        for j in range(epochs): # python 3
            random.shuffle(training_data)
            mini_batches = [training_data[k:k+mini_batch_size] for k in range(0, n, mini_batch_size)] # python 3
